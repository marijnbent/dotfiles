...
sudo scutil --set ComputerName "MacBook Marijn"
sudo scutil --set HostName "MacBook Marijn"
sudo scutil --set LocalHostName "MacBook Marijn"
sudo defaults write /Library/Preferences/SystemConfiguration/com.apple.smb.server NetBIOSName -string "MacBook Marijn"
...
# Disable default applications when a DVD/CD is inserted
...
# Use column view (three-panel) in all Finder windows by default
# Four-letter codes for other view modes: `icnv`, `Nlsv`, `glyv`
...
# Finder: Desktop and Downloads folders - list view, sorted by date (newest first)
# Per-folder view settings require briefly opening each folder via AppleScript
osascript << 'APPLESCRIPT'
tell application "Finder"
    set dateDescFolders to {folder "Desktop" of home, folder "Downloads" of home}
    repeat with f in dateDescFolders
        open f
        set w to front window
        set current view of w to list view
        set sort column of list view options of w to modification date column
        close w
    end repeat

    set nameAscFolders to {folder "Applications" of startup disk}
    repeat with f in nameAscFolders
        open f
        set w to front window
        set current view of w to list view
        close w
    ...
    set columnViewFolders to {folder "Projects" of home}
    repeat with f in columnViewFolders
        open f
        delay 1
        set w to front window
    ...
    set iCloudDrive to folder ((POSIX file (POSIX path of (path to home folder) & "Library/Mobile Documents/com~apple~CloudDocs")) as alias)
    open iCloudDrive
    delay 1
    set w to front window
    set current view of w to column view
...
# Set Zed as default editor for text and code files
...
# Set Ghostty as default for terminal-runnable files
